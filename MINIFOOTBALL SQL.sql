...
BEGIN
    DELETE FROM OWNER 
    WHERE G_NO = (SELECT G_NO FROM GAMER WHERE G_SESSIONID = V_G_SESSIONID) 
    AND P_NO = V_P_NO;
...
CREATE OR REPLACE TRIGGER AFTER_SELLPLAYER
AFTER DELETE ON OWNER
FOR EACH ROW
DECLARE
    V_P_PRICE NUMBER;
BEGIN
    SELECT NVL(P_PRICE, 99999999) INTO V_P_PRICE FROM PLAYER WHERE P_NO = :OLD.P_NO;
    UPDATE GAMER 
    SET G_BALANCE = G_BALANCE + V_P_PRICE 
    WHERE G_NO = :OLD.G_NO;
END;
/
...
IS
BEGIN
    INSERT INTO OWNER VALUES((SELECT G_NO FROM GAMER WHERE G_SESSIONID = V_G_SESSIONID), V_P_NO);
END;
/

CREATE OR REPLACE TRIGGER BEFORE_BUYPLAYER
BEFORE INSERT ON OWNER
FOR EACH ROW
DECLARE
    V_P_PRICE NUMBER;
BEGIN
    SELECT NVL(P_PRICE, 99999999) INTO V_P_PRICE FROM PLAYER WHERE P_NO = :NEW.P_NO;
